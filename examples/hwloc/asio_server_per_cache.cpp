...
static tmc::task<void> accept(tmc::ex_asio& ex, uint16_t Port) {
  ...
  int one = 1;
  // TODO make sure this works on windows (probably have to remove SO_REUSEPORT)
  setsockopt(
    acceptor.native_handle(), SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
...
int main(int argc, char* argv[]) {
  // TODO query and tiny_vec need to be in public headers
  ...
  if (argc > 1) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      exCpus[i].set_topology_filter(f);
      exCpus[i].set_thread_occupancy(1.5f, tmc::topology::CpuKind::PERFORMANCE);
      exCpus[i].init();
    }
