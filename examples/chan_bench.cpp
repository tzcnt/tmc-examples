// A benchmark for the throughput of tmc::mutex_queue.
...
using queue_t = tmc::mutex_queue<std::optional<size_t>>;

static tmc::task<void> producer(queue_t* queue, size_t count, size_t base) {
  for (size_t i = 0; i < count; ++i) {
    co_await queue->push_back(base + i);
  }
}
...
static tmc::task<result> consumer(queue_t* queue) {
  size_t count = 0;
  size_t sum = 0;

  while (auto data = co_await queue->pop_front()) {
    ++count;
    sum += *data;
  }
...
int main() {
  ...
  std::printf(
    "queue_bench: %zu threads | %s elements\n",
  ...
  return tmc::async_main([]() -> tmc::task<int> {
    ...
    for (size_t consCount = 1; consCount <= 10; ++consCount) {
      for (size_t prodCount = 1; prodCount <= 10; ++prodCount) {
        ...
        for (size_t i = 0; i < consCount; ++i) {
          co_await queue.push_back(std::nullopt);
        }
