...
static inline void enqueue(queue_t* queue, const void* item) {
  size_t head =
    atomic_fetch_add_explicit(&queue->head, 1, memory_order_acq_rel);
  slot_t* slot = &queue->slots[head % SLOTS];
  while ((head / SLOTS) * 2 != atomic_load_explicit(
                                 &slot->turn, memory_order_acquire
                               )) { /* busy-wait */
  }
  memcpy(slot->data, item, SLOT);
  atomic_store_explicit(
    &slot->turn, (head / SLOTS) * 2 + 1, memory_order_release
  );
}

static inline void dequeue(queue_t* queue, void* item) {
  size_t tail =
    atomic_fetch_add_explicit(&queue->tail, 1, memory_order_acq_rel);
  slot_t* slot = &queue->slots[tail % SLOTS];
  while ((tail / SLOTS) * 2 + 1 != atomic_load_explicit(
                                     &slot->turn, memory_order_acquire
                                   )) { /* busy-wait */
  }
  memcpy(item, slot->data, SLOT);
  atomic_store_explicit(
    &slot->turn, (tail / SLOTS) * 2 + 2, memory_order_release
  );
}
...
static inline bool try_dequeue(queue_t* queue, void* item) {
  ...
  for (;;) {
    ...
    if ((tail / SLOTS) * 2 + 1 ==
        atomic_load_explicit(&slot->turn, memory_order_acquire)) {
      if (atomic_compare_exchange_strong_explicit(
            &queue->tail, &tail, tail + 1, memory_order_acq_rel,
            memory_order_acquire
          )) {
        memcpy(item, slot->data, SLOT);
        atomic_store_explicit(
