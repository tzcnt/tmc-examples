...
//
// This is the same timeout-based cancellation scenario as cancellation.cpp, but
// expressed declaratively. Whereas cancellation.cpp manually drives
// `spawn_tuple().result_each()`, inspects each ready index, and calls cancel()
// on the loser from the awaiting task, `tmc::select()` collapses all of that
// into a single `co_await`:
//   - it returns the winner's result in a std::variant (variant.index() tells
//     you which operation won),
//   - it invokes the canceller of every other operation for you, and
//   - it waits for those cancelled operations to finish before returning.
//
// Because the thing you cancel (here, the asio::steady_timer) is not the same
// object as the awaitable returned by async_wait(), each awaitable is paired
// with a canceller using `tmc::cancellable()`. Since steady_timer exposes a
// `.cancel()` method, the timer can be passed directly as the canceller; for
// objects without such a method, pass a lambda instead, e.g.
// `tmc::cancellable(some.async_op(tmc::aw_asio), [&]{ some.stop(); })`.
  ...
  return tmc::async_main([]() -> tmc::task<int> {
    ...
    for (auto mainOpDuration : mainOpDurations) {
      ...
      std::variant<std::tuple<error_code>, std::tuple<error_code>> result =
        co_await tmc::select(
          // This demonstrates 2 overloads of tmc::cancellable():
          // #1 - 2nd parameter is a lambda that cancels the operation when invoked.
          tmc::cancellable(
