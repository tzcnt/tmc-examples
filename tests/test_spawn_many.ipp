...
template <int N> tmc::task<void> spawn_many_static_sized_iterator() {
...
template <int N> tmc::task<void> spawn_many_static_bounded_iterator() {
...
template <int N> tmc::task<void> spawn_many_dynamic_known_sized_iterator() {
...
template <int N> tmc::task<void> spawn_many_range() {
  auto iter = iter_of_dynamic_known_size<N>();

  // The template parameter N to spawn_many is not provided.
  // This overload will produce a right-sized output vector
  // (internally calculated from iter.end() - tasks.iter())
...
template <int N> tmc::task<void> spawn_many_dynamic_unknown_sized_iterator() {
  auto iter = iter_of_dynamic_unknown_size<N>();
...
template <int N> tmc::task<void> spawn_many_unknown_sized_range() {
  auto iter = iter_of_dynamic_unknown_size<N>();
...
template <int N> tmc::task<void> spawn_many_dynamic_bounded_iterator() {
  // In this example, we do not know the exact number of tasks that iter could
...
TEST_F(CATEGORY, spawn_many_static_sized_iterator) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_static_sized_iterator<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_static_bounded_iterator) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_static_bounded_iterator<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_dynamic_known_sized_iterator) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_dynamic_known_sized_iterator<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_range) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_range<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_unknown_sized_range) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_unknown_sized_range<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_dynamic_unknown_sized_iterator) {
  test_async_main(ex(), []() -> tmc::task<void> {
    co_await spawn_many_dynamic_unknown_sized_iterator<5>();
  }());
}

TEST_F(CATEGORY, spawn_many_dynamic_bounded_iterator) {
  test_async_main(ex(), []() -> tmc::task<void> {
