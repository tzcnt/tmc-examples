...
static_assert(
  sizeof(tmc::detail::cancellable_pair<
         std::unique_ptr<int>, tmc::detail::cancel_self_t>) ==
    sizeof(std::unique_ptr<int>),
  "cancel_self_t should occupy no storage in the cancellable_pair"
);
...
// Overload 1 with an *awaitable* (task) canceller: select() co_awaits the task
// to cancel the loser (an asynchronous cancel). The task releases the blocked
// loser, which is then drained.
TEST_F(CATEGORY, select_task_canceller) {
  test_async_main(ex(), []() -> tmc::task<void> {
    ...
    std::variant<std::monostate, std::monostate> result = co_await tmc::select(
      tmc::cancellable(winner(), [] {}),
      tmc::cancellable(loser(block), cancelTask(block)) // bare task canceller
    );
...
// The bare task canceller of the *winner* is never awaited. select() must
// discard it (free its coroutine frame) rather than leak it - tmc::task asserts
// in its destructor unless consumed. The loser is cancelled by a plain lambda.
TEST_F(CATEGORY, select_task_canceller_discarded_when_winner) {
  test_async_main(ex(), []() -> tmc::task<void> {
    atomic_awaitable<int> block(1);

    auto winner = []() -> tmc::task<void> { co_return; };
    auto neverRun = []() -> tmc::task<void> { co_return; };
    auto loser = [](atomic_awaitable<int>& B) -> tmc::task<void> { co_await B; };

    std::variant<std::monostate, std::monostate> result = co_await tmc::select(
      tmc::cancellable(winner(), neverRun()), // task canceller; winner -> discarded
      tmc::cancellable(loser(block), [&] { block.inc(); })
    );
...
template <IsCoroutineModeOp Awaitable> struct awaitable_traits<Awaitable> {
  ...
  static void set_result_ptr(
    self_type& a, tmc::detail::result_storage_t<result_type>* ResultPtr
  ) {
