...
TEST_F(CATEGORY, free_function) {
  tmc::util::coro_functor f(free_func);
  f();
...
TEST_F(CATEGORY, lambda) {
  {
    // rvalue
    int result = 0;
    tmc::util::coro_functor f([&result]() { result = 1; });
    f();
    EXPECT_EQ(result, 1);
    EXPECT_EQ(f.is_coroutine(), false);
  }
...
TEST_F(CATEGORY, invokable_struct) {
  {
    ...
    {
      tmc::util::coro_functor f(inv{result, destructor_count});
      EXPECT_EQ(f.is_coroutine(), false);
      f();
    }
    EXPECT_EQ(result, 1);
  ...
  }
  {
    ...
    int destructor_count = 0;
    {
      ...
      tmc::util::coro_functor f(inv_lvalue);
      EXPECT_EQ(f.is_coroutine(), false);
      f();
    }
    EXPECT_EQ(result, 1);
...
TEST_F(CATEGORY, coro) {
  {
    ...
    int result = 0;
    {
      tmc::util::coro_functor f([](int& r) -> tmc::task<void> {
        ++r;
        co_return;
      }(result));
  ...
  }
  {
    ...
    int result = 0;
    {
      ...
      tmc::util::coro_functor f(std::move(t));
      EXPECT_EQ(f.is_coroutine(), true);
      f();
    }
    EXPECT_EQ(result, 1);
