...
/// @brief A C++ concept that lets us distinguish e.g. a @c
/// std::normal_distribution from a @c std::vector
/// @note  Relies on STL distributions defining a @c param_type type that STL
/// containers do not.
template <typename D>
concept Distribution = requires { typename D::param_type; };

/// @brief  The main `xso::generator` class combines a State and a Scrambler to
/// create a PRNG.
/// @tparam State     Provides access to the words of state, and has a @c step()
/// method to advance them.
/// @tparam Scrambler Functor to reduce the State to a single output word (a 32
/// or 64 bit unsigned).
/// @note   Most users will not use this directly but instead reference one of
/// the type aliased generators below.
...
public:
  ...
  /// @brief The state is packed into words of this type (in practice, 32 or 64
  /// bit unsigneds).
  ...
  /// @brief Construct a generator quickly but @b not well from a single
  /// unsigned integer value.
  /// @note  Seeding from a single value is an easy way to get repeatable random
  /// streams.
  explicit generator(word_type s) { seed(s); }

  /// @brief Construct and seed from an iteration of words which are all copied
  /// into the state.
  /// @note  The words shouldn't all be zeros.
  template <typename Iter> explicit generator(Iter b, Iter e) { seed(b, e); }
  ...
  void seed() {
    ...
    // Fill a full seed array with calls to dev() --  may need a couple of dev()
    // calls to fill one word of state.
    std::array<word_type, word_count()> full_state;
    if constexpr (sizeof(word_type) <=
                  sizeof(std::random_device::result_type)) {
      for (auto& word : full_state)
        word = static_cast<word_type>(dev());
    } else {
      for (auto& word : full_state)
        word =
          static_cast<word_type>(static_cast<uint64_t>(dev()) << 32 | dev());
    }

    // std::random_device may be poor so we dd data from a call to a high
    // resolution clock for first word.
    using clock_type = std::chrono::high_resolution_clock;
    auto ticks =
      static_cast<std::uint64_t>(clock_type::now().time_since_epoch().count());

    // However, from call to call, ticks only changes in the low order bits --
    // better scramble things a bit!
    ticks = murmur_scramble64(ticks);

    // Fold the scrambled ticks variable into the first seed word & seed the
    // state.
    full_state[0] ^= static_cast<word_type>(ticks);
    m_state.seed(full_state.cbegin(), full_state.cend());
  }

  /// @brief Fill the state quickly but probably @b not well from a single
  /// unsigned integer value.
  /// @note  Seeding from a single value is an easy way to get repeatable random
  /// streams.
  constexpr void seed(word_type seed) {
    ...
    // Use SplitMix64 to at least put some values in all the state words & seed
    // the state.
    std::array<word_type, word_count()> full_state;
    for (auto& word : full_state)
      word = static_cast<word_type>(split_mix64(sm64_state));
    m_state.seed(full_state.cbegin(), full_state.cend());
  }

  /// @brief Set the state from an iteration of words which are all copied into
  /// the state.
  /// @note  The words shouldn't all be zeros.
  template <typename Iter> constexpr void seed(Iter b, Iter e) {
    m_state.seed(b, e);
  }
  ...
  /// @brief Reduce the current state to get a single @c result_type and then
  ...
  template <typename Iter> constexpr void get_state(Iter dst) const {
    m_state.get_state(dst);
  }

  /// @brief Returns a single integer value from a uniform distribution over @c
  /// [a,b].
  /// @note  No error checking is done and the behaviour is undefined if a > b.
  template <std::integral T> constexpr T sample(T a, T b) {
    return std::uniform_int_distribution<T>{a, b}(*this);
  }

  /// @brief Returns a single real value from a uniform distribution over @c
  /// [a,b).
  /// @note  No error checking is done and the behaviour is undefined if a > b.
  template <std::floating_point T> constexpr T sample(T a, T b) {
  ...
  /// 0.
  template <std::integral T> constexpr T index(T len) {
    return sample(T{0}, len - 1);
  }

  /// @brief Returns a single value from an iteration -- all elements are
  /// equally likely to be returned.
  /// @note  No error checking is done and the behaviour is undefined if e < b.
  template <std::input_iterator T> constexpr auto sample(T b, T e) {
    // Edge case?
  ...
  /// @brief Pick @c n elements from an iteration @c [b,e) without replacement &
  /// put the chosen samples in @c dst.
  ...
  /// @brief Pick @c n elements from a container without replacement & put the
  /// chosen samples in @c dst.
  /// @note  See the documentation for @c std::sample(...) for more details.
  template <typename Src, typename Dst>
  constexpr auto
  sample(const Src& src, Dst dst, std::unsigned_integral auto n) {
    return sample(std::cbegin(src), std::cend(src), dst, n);
  }

  /// @brief Returns a single random variate drawn from a distribution.
  /// @param dist The distribution in question e.g. a @c
  /// std::normal_distribution object.
  constexpr auto sample(Distribution auto& dist) { return dist(*this); }

  /// @brief Pushes @c n samples from a distribution into a destination
  /// iterator.
  /// @param dist The distribution in question e.g. a @c
  /// std::normal_distribution object.
  ...
  constexpr Iter
  sample(Distribution auto& dist, Iter dst, std::unsigned_integral auto n) {
    while (n-- != 0)
      *dst++ = dist(*this);
    return dst;
  }
  ...
  /// @brief Flip a coin,
  /// @param p Probability of getting a true uniform return -- defaults to a
  /// fair 50%.
  ...
  template <typename Container> constexpr void shuffle(Container& container) {
    return shuffle(std::begin(container), std::end(container));
  }

  /// @brief Discard the next @c z iterations in the random number sequence.
  /// @note  We can do much better for large @c z by using one of the @c jump
  /// methods.
  ...
  /// @brief  If the state's characteristic polynomial is c(x) = x^n + p(x)
  /// where degree[p] < n then we
  ///         fill @c dst with the @b precomputed coefficients for p(x) = p_0 +
  ///         p_1 x + ... + p_{n-1}.
  /// @throws If the @c State has no precomputed characteristic coefficients
  /// this fails.
  template <typename Iter>
...
private:
  ...
  /// @brief  The SplitMix64 random number generator -- a simple generator with
  /// 64 bits of state.
...
// It provides access to those words and to a `step()` method that advances the
...
public:
  ...
  /// @brief Set the state from an iteration of words which shouldn't be all
  /// zeros.
  template <typename Iter> constexpr void seed(Iter b, Iter e) {
  ...
  constexpr void step() {
    if constexpr (N == 4) {
      ...
      // hard coded -- this contrasts to xoroshiro state. So if we get to here
      // we don't have a formula that works to advance the state and need to
      // fail out. Get a useful'ish error message by pumping a deliberately
  ...
  /// @param  dst We fill this destination with the the precomputed coefficients
  /// of @b p(x)
  /// @throws If there are no precomputed characteristic coefficients we throw
  /// an error.
  template <typename Iter>
  static constexpr void characteristic_coefficients(Iter dst) {
    static_assert(
      (std::is_same_v<T, uint32_t> && N == 4 && A == 9 && B == 11) ||
      (std::is_same_v<T, uint64_t> && N == 4 && A == 17 && B == 45) ||
      (std::is_same_v<T, uint64_t> && N == 8 && A == 11 && B == 21)
    );
    // In practice we have precomputed the p(x) polynomial for just a few
    // xoshiro's with specific parameters.
    if constexpr (std::is_same_v<T, uint32_t> && N == 4 && A == 9 && B == 11) {
      ...
      std::copy(p.cbegin(), p.cend(), dst);
                         ...
                         B == 45) {
      std::array<T, N> p = {
        0x9d116f2bb0f0f001, 0x280002bcefd1a5e, 0x4b4edcf26259f85,
        0x3c03c3f3ecb19
      };
      std::copy(p.cbegin(), p.cend(), dst);
    } else if constexpr (std::is_same_v<T, uint64_t> && N == 8 && A == 11 &&
                         B == 21) {
      std::array<T, N> p = {0xcf3cff0c00000001, 0x7fdc78d886f00c63,
                            0xf05e63fca6d7b781, 0x7a67058e7bbab6f0,
                            0xf11eef832e32518f, 0x51ba7c47edc758ad,
...
public:
  ...
  /// @brief Read-only access to the i'th state word.
  /// @note  For larger values of N we are using the state array as a ring
  /// buffer!
  ...
  /// @brief Read-only access to the whole state which we copy into @c dst.
  /// @note  For larger values of N we are using the state array as a ring
  /// buffer which needs to be untangled!
  template <typename Iter> constexpr void get_state(Iter dst) const {
  ...
  /// @brief Advance the state by one step.
  constexpr void step() {
    // Depending on the word_count of N we either do an explicit or implicit
    // array shuffle of the state array.
    if constexpr (N == 2)
      simple_step();
    else
      clever_step();
  ...
  static constexpr void characteristic_coefficients(Iter dst) {
    ...
    if constexpr (std::is_same_v<T, uint32_t> && N == 2 && A == 26 && B == 9 &&
      ...
      std::copy(p.cbegin(), p.cend(), dst);
                         ...
                         B == 27 && C == 36) {
      std::array<T, N> p = {0x5cfeb8cc48ddb211, 0xb73e379d035a06dd,
...
private:
  ...
  constexpr void clever_step() {
    ...
    // Step the index of the final word of state -- this shuffles the state
    // array down a slot.
    m_final = i_first;
  }
};
...
// The Scramblers:
//
// A Scrambler is a functor that is passed a State and returns a single unsigned
// output word.
// --------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------
// Non-member functions that are used to "jump" a generator or state far ahead
// in its stream.
// --------------------------------------------------------------------------------------------------------------------
namespace xso::internal {

// Polynomial Reduction:
// Start with internal functions that are used to compute x^J mod c(x) where
// c(x) = x^n + p(x) and degree[p] < n. Re-implements the more general
// `bit::polynomial::reduce` method -- see https://nessan.gitbub.io/bit.
// Repeated here to make `xoshiro/xoroshiro` complete w/o any need to reference
// the `bit` library.
...
constexpr void riffle(
  const std::array<word_type, N>& src, std::array<word_type, N>& lo,
  std::array<word_type, N>& hi
) {
  // We will riffle each word in src into two other words x & y
  word_type x, y;

  // We work through src in reverse order -- this allows the reuse of src for
  // lo! Treating [lo|hi] as contiguous storage, first working back through hi
  // and then back through lo.
  for (std::size_t i = N; i-- > 0;) {
    ...
    if (2 * i + 1 > N) {
      ...
      // Straddling situation where y goes in the first word of hi and x in the
      // last word of lo.
      lo[N - 1] = x;
      hi[0] = y;
    } else {
...
/// @brief  Computes x^e mod c(x) in GF(2) for e = J or 2^J and c(x) = x^n +
/// p(x) where degree[p] < n.
/// @param  p We are passed the coefficients of @b p(x) packed into an an array.
/// @param  J_is_pow2 If true we compute x^(2^J) mod c(x) -- allows e.g. e =
/// 2^100 which overflows a @c std::size_t
/// @return We return the coefficients of r(x) = x^e mod c(x) in the same type
/// of array as @c p.
/// @note   linters will (reasonably) complain that the complexity of this
/// method is rather high!
template <std::unsigned_integral T, std::size_t N>
constexpr std::array<T, N>
reduce(const std::array<T, N>& p, std::size_t J, bool J_is_pow2) {
  // This is similar in spirit/algorithm to `bit::polynomial::reduce` method --
  // see https://nessan.github.io/bit This version is less general as it assumes
  // that the degree of c(x) is a multiple of 32 + 1. NOTE: The `bit` version is
  // more factored and more readable (this method's complexity is high).

  // Constant we use to indicate "no such position"/"not found" and a couple of
  // others.
  constexpr auto npos = static_cast<std::size_t>(-1);
  constexpr T one{1};

  // The polynomial c(x) = x^n + p(x) where p(x) = p_0 + p_1 x + ... + p_{n-1}
  // x^{n-1} and n is:
  ...
  // lambda: Returns the index for the least significant set bit in the argument
  // or `npos` if none set.
  auto lsb = [=](T w) {
    return w == 0 ? npos : static_cast<std::size_t>(std::countr_zero(w));
  };

  // lambda: Returns the index for the most significant set bit in the argument
  // or `npos` if none set.
  auto msb = [=](T w) {
    return static_cast<std::size_t>(std::bit_width(w) - 1);
  };

  // lambda: Returns the first set coefficient in poly or `npos` if the
  // coefficients are all zero.
  auto first_set = [=](const auto& poly) {
    for (std::size_t i = 0; i < N; ++i)
      if (poly[i] != 0)
        return i * bits_per_word + lsb(poly[i]);
    return npos;
  };

  // lambda: Returns the final set coefficient in poly or `npos` if the
  // coefficients are all zero.
  auto final_set = [=](const auto& poly) {
    std::size_t i = N;
    while (i--)
      if (poly[i] != 0)
        return i * bits_per_word + msb(poly[i]);
    return npos;
  };

  // lambda: Returns true if the highest coefficient in `poly` is set (then poly
  // is said to be monic).
  ...
  // lambda: Computes lhs <- lhs + rhs which in GF(2) is equivalent to  lhs <-
  // lhs^rhs.
  ...
  // lambda: Performs a one place shift on the polynomial coefficients stored
  // poly. Shift is to the the right if you think the elements are in vector
  // order [p0,p1,p2,p3] -> [0,p0,p1,p2]. Shift is to the left when you think in
  // bit order [b3,b2,b1,b0] -> [b2,b1,b0,0].
  ...
  // lambda: If degree[poly] < n, this performs poly(x) <- x*poly(x) mod c(x)
  // where c(x) = x^n + p(x).
  ...
  // We precompute x^{n + i} mod c(x) for i = 0, ..., n-1 starting from the
  // known x^n mod c(x) = p. Each x^{n + i} mod c(x) is a word-vector of
  // coefficients and we put the lot into a std::vector.
  ...
  // lambda: If degree[poly] < n, performs: poly(x) <- poly(x)^2 mod c(x) where
  // as usual c(x) = x^n + p(x).
  auto square_step = [&](auto& poly) {
    // Compute poly(x)^2 -- in GF(2) this means interspersing all the
    // coefficients with zeros. We actually riffle poly directly into two arrays
    // lo & hi so that poly(x)^2 = lo(x) + x^n hi(x). This only works because we
    // assume n some multiple of N (i.e. all bits in poly matter). NOTE: Our
    // riffle method above for arrays lets us reuse the poly array for lo.
    riffle(poly, poly, hi);

    // poly(x)^2 mod c(x) now is poly(x) + x^n hi(x) mod c(x) as degree[poly] <
    // n. Add the x^n hi(x) mod c(x) term-by-term noting that at most every
    // second term in hi(x) is 1.
    auto hi_first = first_set(hi);
    if (hi_first != npos) {
      auto hi_final = final_set(hi);
      for (std::size_t i = hi_first; i <= hi_final; i += 2)
        if (test(hi, i))
          add(poly, power_mod[i]);
    }
  };
  ...
  // Case: e = 2^J -- we just do J squaring steps starting from r(x) = x to get
  // to  x^(2^J) mod c(x).
  if (J_is_pow2) {
    set(r, 1);
  ...
  // Case e = J < n: Then x^J mod c(x) = x^J so we can set the appropriate
  // coefficient in r and return.
  if (J < n) {
    set(r, J);
    return r;
  }
  ...
  // Start with r(x) = x mod c(x) which takes care of the most significant
  // binary digit in J.
  set(r, 1);
  J_bit >>= 1;
  ...
  while (J_bit) {
    ...
    // Always do a square step and then a times_x step if necessary (i.e. if
    // current bit in J is set).
    square_step(r);
    if (J & J_bit)
      times_x_step(r);
...
/// @brief  Returns the coefficients for the jump polynomial that jumps the
/// generator/state ahead by N or 2^N steps.
/// @param  N_is_pow2 If true the jump size is 2^N -- allows for e.g. 2^100
/// slots which overflows a @c std::size_t
/// @return If the associated characteristic polynomial is c(x) the jump
/// polynomial is: r(x) = x^N mod c(x).
///         We return the coefficients of @b r(x) in an appropriate sized @c
///         std::array
/// @throw  If the @c State has no precomputed characteristic coefficients this
/// fails.
...
jump_coefficients(std::size_t N, bool N_is_pow2 = false) {
  ...
  // The jump polynomial is x^J mod c(x) -- that computation expects to be
  // handed p(x) where c(x) = x^n + p(x).
...
/// @param  N_is_pow2 If true the jump size is 2^N -- allows for e.g. 2^100
/// slots which overflows a @c std::size_t
/// @return If the state's characteristic polynomial is c(x) the jump polynomial
/// is: r(x) = x^N mod c(x).
///         We return the coefficients of r(x) in an appropriate sized @c
///         std::array
...
/// @brief Jumps state quickly forward by J steps where J can be huge (e.g.
/// 2^100)
/// @param state      The generator or state that needs to be jumped.
/// @param jump_coeff The coefficients of the jump polynomial x^J mod c(x)
/// packed in an array of unsigned words.
/// @note  You get the @c jump_coeff array by first calling the @c
/// jump_coefficients method for the jump in question.
template <typename State>
void jump(
  State& state,
  const std::array<typename State::word_type, State::word_count()>& jump_coeff
) {
  using word_type = typename State::word_type;
  ...
  // Computing the sum r(T).s = r_0.s + r_1.s^1 + ... + r_{n-1} s^{n-1} where s
  // is the current state. T is the state's transition matrix and we compute
...
//
// For parallel processing applications it can be useful to split a single
// random number stream into a number of non-overlapping "partitions". Then
// different computational threads can get their "own set" of random numbers
// without worrying about stream overlaps etc.
//
// The partitions need to be very large so this only works for States where we
// can jump far ahead in an efficient way.
// --------------------------------------------------------------------------------------------------------------------

/// @brief  Partition a random number stream into a number of non-overlapping
/// sub-streams.
/// @tparam State Any state/generator type that we can @c jump ahead in its
/// stream.
template <typename State> class partition {
public:
  /// @brief Constrict a partition for the passed generator/state.
  /// @param state        The parent generator/state already seeded to some
  /// starting point for its state.
  ...
  partition(const State& state, std::size_t n_partitions) : m_state{state} {
    ...
    // std::size_t so we must keep everything in log 2 form. First we find the
    // smallest n such that 2^n >= n_partitions - 1.
    // Note if n_partitions is 128 the following gives n = 7 and does the same
    // if n = 100.
    auto n = static_cast<std::size_t>(std::bit_width(n_partitions - 1));

    // We will create 2^n partitions which is probably more than needed but the
    // wastage is negligible. To create those 2^n partitions we must be able to
    // jump ahead 2^(n_bits - n) steps many times.
    auto power_2 = n_bits - n;

    // Precompute the jump coefficients to advance the generator 2^power_2 steps
...
/// @brief Returns the transition matrix for a state/generator type as a @c
/// bit::matrix.
template <typename State> auto transition_matrix() {
  // The state bits are packed into words of this type (in practice, 32 or 64
  // bit unsigneds).
  using word_type = typename State::word_type;
  ...
  // Create an state instance (this can be a State or a full generator -- just
  // needs to support a few methods).
  State state;
  ...
  for (std::size_t k = 0; k < n_bits; ++k) {
    ...
    // Seed the state from that k'th unit state -- first translating the bits to
    // words.
    ...
    // Grab the resulting state as an array of words and convert that to a
    // bit-vector
...
template <typename State> auto transition_matrix(const State&) {
  // The argument is only used to determine the `State` type.
  return transition_matrix<State>();
}

/// @brief Returns the characteristic polynomial for a state/generator type as a
/// @c bit::polynomial.
/// @note  If the transition matrix is n x n then the return will have
/// word_count n+1 and should be monic.
template <typename State> auto characteristic_polynomial() {
...
template <typename State> auto characteristic_polynomial(const State&) {
  // The argument is only used to determine the `State` type.
  return characteristic_polynomial<State>();
}

/// @brief  Returns a jump polynomial that moves a state/generator type @c J
/// steps ahead in its random number stream.
/// @param  c The precomputed characteristic polynomial for a state/generator
/// type as a @c bit::polynomial
/// @param  N We want to jump by J = N steps or J = 2^N steps (for really huge
/// jumps).
/// @param  N_is_pow2 If true we want to jump by 2^N steps -- allows for say J =
/// 2^100 which overflows normal ints.
/// @return Returns the jump polynomial x^J mod c(x) as a @c bit::polynomial
template <std::unsigned_integral Block, typename Allocator>
auto jump_polynomial(
  const bit::polynomial<Block, Allocator>& c, std::size_t N,
...
/// @brief Jumps a state/generator ahead in its random number stream by @c J
/// steps.
/// @param jump_poly The precomputed bit-polynomial x^J mod c(x) where c(x) is
/// the characteristic polynomial.
/// @note  You get @c jump_poly by first calling the @c jump_polynomial method
/// for the jump in question.
template <typename State, std::unsigned_integral Block, typename Allocator>
void jump(State& state, const bit::polynomial<Block, Allocator>& jump_poly) {
  std::array<typename State::word_type, State::word_count()> sum;

  // Computing [r_0 + r_1 T + ... + r_{m-1} T^{m-1}].s where s is the current
  // state and r is the jump polynomial. T is the state's transition matrix so
  // we can compute s^{i+1} = T.s^i using the step() method.
  sum.fill(0);
