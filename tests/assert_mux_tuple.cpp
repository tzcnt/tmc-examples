  ...
  EXPECT_DEATH(
    {
      ...
      test_async_main(ex, []() -> tmc::task<void> {
        [[maybe_unused]] tmc::mux_tuple mux(
          []() -> tmc::task<void> { co_return; }()
        );
        co_return;
      }());
    },
...
// restart<I>() may only be called on a slot whose previous result has already
// been consumed by co_await. Restarting a still-pending slot trips the assert.
