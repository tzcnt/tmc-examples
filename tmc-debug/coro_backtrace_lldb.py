...
class CoroFrame(ScriptedFrame):
    ...
    def get_function_name(self):
        try:
            ...
            if symbol.IsValid():
                name = symbol.GetName()
                prefix = "[fork] [async] " if self._is_fork else "[async] "
                return prefix + name
        except Exception as e:
...
def _tmc_coro_locals_b64(thread):
    """Return base64-encoded JSON of each async coroutine frame's locals, in continuation
    order: [{"function", "locals": [{"name","value","type","numChildren"}, ...]}, ...].

    This reuses the exact chain-walk and CoroFrame.get_variables() logic the frame provider
    uses, so the Nth entry corresponds to the Nth [async] frame in the backtrace. It exists as
    a stopgap: LLDB 22.x does not yet invoke get_variables() for the Variables pane (LLVM PR
    #178575), so a DAP proxy calls this to populate synthetic-frame locals until that ships.
    """
    import base64, json
    out = []
    try:
        ...
        for i in range(thread.GetNumFrames()):
            ...
            pv = f.GetValueForVariablePath("__promise")
            g = False
            if not pv.IsValid():
                pv = f.GetValueForVariablePath("_Coro_promise")
                g = True
            if pv.IsValid():
                c = pv.GetChildMemberWithName("customizer")
                if c.IsValid():
                    customizer, is_gcc = c, g
                    break
        if customizer is not None:
            prov = object.__new__(CoroFrameProvider)
            prov.thread = thread
            prov.target = target
            prov.process = process
            prov._args = None
            prov._frames = []  # _get_coro_frames reads len(self._frames) for frame ids
            for entry in prov._get_coro_frames(customizer, is_gcc):
                cf = entry.get("coro_frame")
                if cf is None:
                    continue
                locs = []
                vl = cf.get_variables(None)
                for j in range(vl.GetSize()):
                    v = vl.GetValueAtIndex(j)
                    ...
                    locs.append({
                        ...
                        "type": v.GetTypeName() or "",
                        "numChildren": v.GetNumChildren(),
                    })
                out.append({"function": cf.get_function_name(), "locals": locs})
    except Exception as e:
        debug_log(f"_tmc_coro_locals_b64 error: {e}")
    return base64.b64encode(json.dumps(out).encode("utf-8")).decode("ascii")


def tmc_coro_locals_command(debugger, command, exe_ctx, result, internal_dict):
    """`tmc-coro-locals`: print base64 JSON of the in-context thread's async-frame locals.

    Uses the execution-context signature so the thread is taken from the evaluating frame
    (e.g. the DAP frameId the proxy evaluates in), not the globally selected thread - which
    in a multi-threaded program is usually a different thread than the stopped coroutine's.
    """
    thread = None
    try:
        ...
        arg = (command or "").strip()
        if arg:
            # The frontend passes the DAP thread id, since a repl command does not honor the
            # evaluate frameId's thread context.
            tid = int(arg, 0)
        ...
        debug_log(f"tmc-coro-locals: {e}")
    result.PutCString(_tmc_coro_locals_b64(thread))
...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand(
        f"command script add -f {__name__}.tmc_coro_locals_command tmc-coro-locals"
    )
    print(
